  ...
  var CesiumMeasures = (function (Cesium) {
    ...
    _.prototype = {
      ...
      computePolygonCenter: function (cartesianPositions) {
        if (
          !Cesium.defined(cartesianPositions) ||
      ...
      drawLineMeasureGraphics: function (options = {}) {
        ...
        if (this._viewer && options) {
          ...
          _handlers.setInputAction(function (movement) {
            ...
            //鼠标提示信息
            if (labelEntityOne)
              $this._drawLayer.entities.remove(labelEntityOne);
          ...
          function _addInfoPoint(position) {
            ...
            _labelEntity.point = {
              ...
              show:
                JSON.stringify(style?.point?.show) == "false" ? false : true,
            };
            if (measure) {
              distance = $this.getPositionDistance(
                $this.transformCartesianArrayToWGS84Array(positions)
              );
              distance = (distance * 10916.0) / 40075.0;
              _labelEntity.label = {
                text: (distance / 1000).toFixed(4) + "公里",
                show: true,
                showBackground: true,
                font: "14px monospace",
                horizontalOrigin: Cesium.HorizontalOrigin.LEFT,
      ...
      drawAreaMeasureGraphics: function (options = {}) {
        ...
        if (this._viewer && options) {
          ...
          function create() {
            if (style?.line) {
              _polygonEntity.polyline = {
                ...
                material:
                  style?.line?.material || Cesium.Color.BLUE.withAlpha(0.8),
                clampToGround: clampToGround || false,
              };
            ...
            _polygonEntity.polygon = {
              ...
              material:
                style?.polygon?.material || Cesium.Color.WHITE.withAlpha(0.1),
          ...
          function _addInfoPoint(position) {
            ...
            _labelEntity.point = {
              pixelSize:
                style?.centerPoint?.pixelSize || style?.point?.pixelSize || 10,
              outlineColor:
                style?.centerPoint?.outlineColor ||
                style?.point?.outlineColor ||
                Cesium.Color.BLUE,
              outlineWidth:
                style?.centerPoint?.outlineWidth ||
                style?.point?.outlineWidth ||
                0,
              color:
                style?.centerPoint?.color ||
                style?.point?.color ||
                Cesium.Color.WHITE,
            };
            if (measure) {
              area = $this.getPositionsArea(
                $this.transformCartesianArrayToWGS84Array(positions)
              );
              area = area * (10916.0 / 40075.0);
              // 获取原始位置
              var originalPosition = _labelEntity.position.getValue(
      ...
      computeRectanglePos: function (lTop, rBottom) {
        ...
        let rightTop2 = Cesium.Cartesian3.fromDegrees(
          rightTop.lng,
          rightTop.lat
        );
        // 计算左下角的坐标
        ...
        let leftBottom2 = Cesium.Cartesian3.fromDegrees(
          leftBottom.lng,
          leftBottom.lat
        );
        positions.push(
          ...
          _handler.setInputAction(function (movement) {
            ...
            if (cartesian && cartesian.x) {
              if (positions.length == 0) {
                ...
                let otherPos = $this.computeRectanglePos(
                  positions[0],
                  cartesian
                );
      ...
      drawCircleMeasureGraphics: function (options = {}) {
        ...
        if (this._viewer && options) {
          ...
          _handler.setInputAction(function (movement) {
            ...
            if (cartesian && cartesian.x) {
              if (positions.length == 0) {
                ...
                _polygonEntity.position = new Cesium.CallbackProperty(
                  function () {
                ...
                if (typeof options.callback === "function") {
                  options.callback(
                    {
                      center: $this.transformCartesianArrayToWGS84Array([
                        center,
                      ]),
                      radius: radius,
                    },
          ...
          _handler.setInputAction(function (movement) {
            ...
            radius = Number(
              $this.getPositionDistance(
                $this.transformCartesianArrayToWGS84Array([center, cartesian])
              )
            );
            // 计算圆形的面积
            ...
            if (measure) {
              ...
              _labelEntity.label = {
                text: (circleArea / 1000000.0).toFixed(4) + "平方公里",
            ...
            _labelEntityOne.label = {
              text:
                positions.length > 0 ? "再次单机后结束绘制" : "单击打开始绘制",
              show: true,
              showBackground: true,
              font: "14px monospace",
              horizontalOrigin: Cesium.HorizontalOrigin.LEFT,
        ...
        function create() {
          _polygonEntity.ellipse = {
            ...
            height: 0, //
            material:
              style?.circle?.material || Cesium.Color.RED.withAlpha(0.5),
            outline: true, // 显示边框
